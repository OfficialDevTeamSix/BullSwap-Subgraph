...
import { updatePairDayData, updateTokenDayData, updateBullDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let bull = BullFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  bull.totalLiquidityBNB = bull.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  bull.totalLiquidityBNB = bull.totalLiquidityBNB.plus(trackedLiquidityBNB);
  bull.totalLiquidityUSD = bull.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  bull.totalTransactions = bull.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let bull = BullFactory.load(FACTORY_ADDRESS);
  bull.totalVolumeUSD = bull.totalVolumeUSD.plus(trackedAmountUSD);
  bull.totalVolumeBNB = bull.totalVolumeBNB.plus(trackedAmountBNB);
  bull.untrackedVolumeUSD = bull.untrackedVolumeUSD.plus(derivedAmountUSD);
  bull.totalTransactions = bull.totalTransactions.plus(ONE_BI);
  ...
  let bullDayData = updateBullDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  bullDayData.dailyVolumeUSD = bullDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  bullDayData.dailyVolumeBNB = bullDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  bullDayData.dailyVolumeUntracked = bullDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  bullDayData.save();
